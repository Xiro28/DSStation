...
public:
	...
	void exec();
	template<const cpu_id_t PROCNUM, const u32 sz> void doCopy();
	void doPause();
	void doStop();
	void doSchedule();
...
struct DmaAddressInfo {
	u32 offset;
	u32 chan;  
	u32 regnum;
};

constexpr DmaAddressInfo computeDmaAddressInfo(DmaRegister adr) {
    static constexpr u32 DMA_REG_SIZE = 12;
    static constexpr u32 DMA_REG_SHIFT = 2;

    constexpr u32 DMA_CONTROL_MIN = static_cast<u32>(DmaRegister::DMA_CONTROL_MIN);
    const u32 adr_value = static_cast<u32>(adr);

    const u32 offset = adr_value - DMA_CONTROL_MIN;
    return DmaAddressInfo{
        offset,
        offset / DMA_REG_SIZE,
        (offset % DMA_REG_SIZE) >> DMA_REG_SHIFT
    };
}
