...
#define ME_EDRAM_MAIN_GPU     (ME_EDRAM_BASE + (192*256*4) + 256)
#define ME_EDRAM_SUB_GPU      (ME_EDRAM_MAIN_GPU + (sizeof(GPU) * 2))
...
__attribute__((noinline, aligned(4))) static int meLoop()
{
    ...
    volatile NDS_Screen main_gpu {.gpu = (GPU*)ME_EDRAM_MAIN_GPU, .offset = 0};
    volatile NDS_Screen sub_gpu  {.gpu = (GPU*)ME_EDRAM_SUB_GPU,  .offset = 512};

    do
    {
        while(syncDrawSc == 0){
            meDCacheInvalidRange((u32)MainScreen.gpu, sizeof(GPU));
            meDCacheInvalidRange((u32)SubScreen.gpu, sizeof(GPU));
            meDCacheInvalidRange((u32)&MMU.LCDCenable, sizeof(MMU.ARM9_VMEM) + ((u32)MMU.ARM9_VMEM - (u32)MMU.LCDCenable));
            meDCacheInvalidRange((u32)&MMU.ARM9_LCD, sizeof(MMU.ARM9_LCD));
