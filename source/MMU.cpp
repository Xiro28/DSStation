...
//MMU_struct *_MMU __attribute__((aligned(16))) = (MMU_struct*) (0x40000000 | (u32)memalign(16, sizeof(MMU_struct)));
MMU_struct *_MMU = (MMU_struct*)memalign(64, sizeof(MMU_struct));
MMU_struct &MMU __attribute__((aligned(64)))= *_MMU;
...
static FORCEINLINE u32 MMU_LCDmap(u32 addr, bool& unmapped, bool& restricted)
{
	...
	if(addr >= 0x03000000 && addr < 0x04000000)
	{
		...
		addr &= 0x00FFFFFF;
		if constexpr (PROCNUM == ARMCPU_ARM7)
		{
	...
	if(addr < 0x06000000 || addr >= 0x07000000) return addr;
...
template<int bank>
static inline void MMU_vram_lcdc()
{
...
template<int bank>
static inline void MMU_vram_arm9(const int offset)
{
	for(int i=0;i<vram_bank_info[bank].num_pages;i++)
	{
		int page = vram_bank_info[bank].page_addr+i;
...
template <int t>
constexpr void map_unexpected(){
	constexpr int types[] = {VRAM_PAGE_ABG,VRAM_PAGE_BBG,VRAM_PAGE_AOBJ,VRAM_PAGE_BOBJ};
	constexpr int sizes[] = {32,8,16,8};

	constexpr int size = sizes[t]; //in 16KB pages
	constexpr int basePage = types[t]; //base page in the vram_arm

	constexpr int mask = size - 1;

	for(int i=size;i<128;i++)
	{
		const int page = basePage + i;
		vram_arm9_map[page] = vram_arm9_map[basePage+(i&mask)];
	}
}

template <u8 block>
static inline void MMU_VRAMmapControl(u8 VRAMBankCnt)
{
	...
	if constexpr(block >= VRAM_BANK_E)
	{
...
void MMU_Reset()
{
	...
	*/
	...
	printf("MMU rtc reset\n");
	rtcInit();
	partie = 1;

	printf("MMU slot1 reset\n");
	slot1_Reset();

	printf("MMU slot2 reset\n");
	slot2_Reset();

	printf("MMU mic reset\n");
...
constexpr uint32_t isqrt_constexpr(uint64_t n) {
    if (n < 2)
        return static_cast<uint32_t>(n);
    uint64_t x = n;
    uint64_t y = (x + 1) >> 1; // (x + 1) / 2
    while (y < x) {
        x = y;
        y = (x + n / x) >> 1;
    }
...
constexpr size_t SQRT_TABLE_SIZE = 2000;
constexpr std::array<uint32_t, SQRT_TABLE_SIZE> generateSqrtTable() {
    std::array<uint32_t, SQRT_TABLE_SIZE> table = {};
    for (size_t i = 0; i < SQRT_TABLE_SIZE; ++i) {
        table[i] = isqrt_constexpr(i);
    }
    return table;
}
...
static void execsqrt() {
	...
	if (mode) { 
		...
		//ret = isqrt(v);
		if (v < SQRT_TABLE_SIZE) {
            ret = sqrt_table[static_cast<size_t>(v)];
        } else {
            // Fallback: try to use the dynamic cache.
            ret = cached_sqrt[v];
            if (!ret && v != 0) {
                ret = isqrt(v);
                cached_sqrt[v] = ret;
...
template<int PROCNUM, u32 addr> static void REG_IF_WriteByte(u8 val)
{
	...
	if constexpr (addr==2) {
		if constexpr (PROCNUM==ARMCPU_ARM9)
			val &= ~0x20;
		else
			val &= ~0x00;
	}
...
template<int PROCNUM, int addr> static void REG_IF_WriteWord(u16 val)
{
...
template <typename T, const cpu_id_t proc>
T * GetMemPtr(u32 addr, const u32 inc){

	//if (inc == 0) return nullptr;

	if constexpr (proc==ARM9)
	{
		if(addr<0x02000000) return nullptr; //itcm
		if((addr&(~0x3FFF)) == MMU.DTCMRegion) return nullptr; //dtcm
	}

	//printf("addr: %x\n", (addr >> 24));

	switch (static_cast<MemoryRegion>(addr >> 24)) {

		case MemoryRegion::VRAM:
		case MemoryRegion::PAL:
		{
			bool unmapped, restricted; 
			addr = MMU_LCDmap<proc>(addr, unmapped, restricted);
			if (unmapped || restricted) return nullptr;
		}
        case MemoryRegion::MainRAM:
			if(proc==ARM9 && addr>>12 == 0x21d7) return nullptr;

			return reinterpret_cast<T*>(MMU.MMU_MEM[proc][addr >> 20] + (addr & MMU.MMU_MASK[proc][addr>>20]));
       
        case MemoryRegion::OAM:
            return reinterpret_cast<T*>(&MMU.ARM9_OAM[addr & 0x7ff]);

		case MemoryRegion::WRAM:
			return reinterpret_cast<T*>(MMU.SWIRAM[addr & 0x7FFF]);

        case MemoryRegion::IO:
		case MemoryRegion::ITCM:
        ...
        default:
			//printf("GetMemPtr: Unhandled region %x\n", addr >> 24);
            return nullptr;
    }
...
template<typename T, const cpu_id_t PROCNUM>
void TransferMemory(u32& saddr, u32& daddr, int todo, const T srcinc, const T dstinc) {
	extern void* memcpy_vfpu( void* dst, void* src, unsigned int size );
 	constexpr int SZ = sizeof(T);
  	//static_assert(SZ == 2 || SZ == 4, "TransferMemory only supports 2- or 4-byte types.");

    // Get mapped pointers. (Assumes GetMemPtr aligns addresses appropriately.)
	...
	if (fallback) {
        for (int i = todo; i > 0; --i) {
            T temp;
            if constexpr (SZ == 4) {
                temp = _MMU_read32<PROCNUM, MMU_AT_DMA>(saddr);
				_MMU_write32<PROCNUM, MMU_AT_DMA>(daddr, temp);
            } else if constexpr (SZ == 2) {
                temp = _MMU_read16<PROCNUM, MMU_AT_DMA>(saddr);
				_MMU_write16<PROCNUM, MMU_AT_DMA>(daddr, temp);
            }
		...
		// This dma seems useless (it copies data to a fixed destination). Probably is used for audio, but I'm not sure...
		// TODO: check this
		/*if (dstinc == 0 && srcinc != 0){

			saddr += srcinc * todo;

			T val = src_ptr ? *(src_ptr + todo) : 0;

			if (!src_ptr)
				val = SZ == 4 ? _MMU_read32<PROCNUM, MMU_AT_DMA>(saddr) : _MMU_read16<PROCNUM, MMU_AT_DMA>(saddr);

			if (dst_ptr) *dst_ptr = val;
			else{
				if constexpr (SZ == 4) {
					_MMU_write32<PROCNUM, MMU_AT_DMA>(daddr, val);
				} else if constexpr (SZ == 2) {
					_MMU_write16<PROCNUM, MMU_AT_DMA>(daddr, val);
				}
			}
		
		}else if (srcinc == 0 && dst_ptr && dstinc > 0){
			
			T val = src_ptr ? *src_ptr : 0;

			if (!src_ptr)
				val = SZ == 4 ? _MMU_read32<PROCNUM, MMU_AT_DMA>(saddr) : _MMU_read16<PROCNUM, MMU_AT_DMA>(saddr);

			memset(dst_ptr, val, todo);
			daddr += dstinc * todo;
		}
		else */
		if (src_ptr && dst_ptr) {
			...
			if (srcinc == dstinc) {
				...
				memcpy_vfpu(dst_ptr, src_ptr, static_cast<size_t>(todo) * SZ);
			...
			for (int i = todo; i > 0; --i) {
				if constexpr (SZ == 4) {
					_MMU_write32<PROCNUM, MMU_AT_DMA>(daddr, *src_ptr);
				} else if constexpr (SZ == 2) {
					_MMU_write16<PROCNUM, MMU_AT_DMA>(daddr, *src_ptr);
				}
				daddr += dstinc;
				src_ptr += incr_src;
			}
			...
			for (int i = todo; i > 0; --i) {
				if constexpr (SZ == 4) {
					*dst_ptr = _MMU_read32<PROCNUM, MMU_AT_DMA>(saddr);
				} else if constexpr (SZ == 2) {
					*dst_ptr = _MMU_read16<PROCNUM, MMU_AT_DMA>(saddr);
				}
				saddr += srcinc;
				dst_ptr += incr_dst;
			}
...
template<const cpu_id_t PROCNUM, const u32 sz>
void DmaController::doCopy()
{
	...
	if constexpr(PROCNUM == ARM9) 
		if(todo == 0) todo = 0x200000; //according to gbatek.. we've verified this behaviour on the arm7
	...
	if constexpr (sz == 4) {	
		time_elapsed = (_MMU_accesstime<PROCNUM, MMU_AT_DMA, 4, MMU_AD_READ, TRUE>(src, true) * todo) + 
						(_MMU_accesstime<PROCNUM, MMU_AT_DMA, 4, MMU_AD_WRITE, TRUE>(dst, true) * todo);
	} else if constexpr (sz == 2) {
		time_elapsed = (_MMU_accesstime<PROCNUM, MMU_AT_DMA, 2, MMU_AD_READ, TRUE>(src, true) * todo) + 
						(_MMU_accesstime<PROCNUM, MMU_AT_DMA, 2, MMU_AD_WRITE, TRUE>(dst, true) * todo);
	}

	// check if we can skip the DMA
	switch (static_cast<MemoryRegion>(daddr >> 24)) {
        case MemoryRegion::ITCM:
        case MemoryRegion::Unused:
			dst += dstinc * todo;
			src += srcinc * todo;
		...
		default:
			if (daddr&(~0x3FFF) == MMU.DTCMRegion || (saddr&(~0x3FFF) == MMU.DTCMRegion)) {
				...
				TransferMemory<uint32_t, PROCNUM>(src, dst, todo, srcinc, dstinc);
			} else if constexpr (sz == 2) {
				TransferMemory<uint16_t, PROCNUM>(src, dst, todo, srcinc, dstinc);
			}
	}
...
void FASTCALL _MMU_ARM9_write08(u32 adr, u8 val)
{
	...
	if ((adr >> 24) == 4)
	{
		...
		switch(adr)
		{
			...
			case REG_VRAMCNTA:
				MMU_VRAMmapControl<0>(val);
			break;
			case REG_VRAMCNTB:
				MMU_VRAMmapControl<1>(val);
			break;
			case REG_VRAMCNTC:
				MMU_VRAMmapControl<2>(val);
			break;
			case REG_VRAMCNTD:
				MMU_VRAMmapControl<3>(val);
			break;
			case REG_VRAMCNTE:
				MMU_VRAMmapControl<4>(val);
			break;
			case REG_VRAMCNTF:
				MMU_VRAMmapControl<5>(val);
			break;
			case REG_VRAMCNTG:
				MMU_VRAMmapControl<6>(val);
			break;
			case REG_WRAMCNT:
				MMU_VRAMmapControl<7>(val);
			break;
			case REG_VRAMCNTH:
				MMU_VRAMmapControl<8>(val);
			break;
			case REG_VRAMCNTI:
				MMU_VRAMmapControl<9>(val);
			break;
...
void FASTCALL _MMU_ARM9_write16(u32 adr, u16 val)
{
	...
	if ((adr >> 24) == 4)
	{
		...
		case REG_DISPA_DISP3DCNT: writereg_DISP3DCNT(16,adr,val); return;
			...
			case REG_VRAMCNTA:
				MMU_VRAMmapControl<0>(val& 0xFF);
				MMU_VRAMmapControl<1>(val >> 8);
			break;
			case REG_VRAMCNTC:
				MMU_VRAMmapControl<2>(val& 0xFF);
				MMU_VRAMmapControl<3>(val >> 8);
			break;
			case REG_VRAMCNTE:
				MMU_VRAMmapControl<4>(val& 0xFF);
				MMU_VRAMmapControl<5>(val >> 8);
			break;
			case REG_VRAMCNTG:
				MMU_VRAMmapControl<6>(val& 0xFF);
				MMU_VRAMmapControl<7>(val >> 8);
			break;
			case REG_VRAMCNTH:
				MMU_VRAMmapControl<8>(val& 0xFF);
				MMU_VRAMmapControl<9>(val >> 8);
			break;
...
void FASTCALL _MMU_ARM9_write32(u32 adr, u32 val)
{
	...
	if ((adr >> 24) == 4)
	{
		...
		switch(adr)
		{
			...
			case REG_VRAMCNTA:
				MMU_VRAMmapControl<0>(val & 0xFF);
				MMU_VRAMmapControl<1>((val >> 8) & 0xFF);
				MMU_VRAMmapControl<2>((val >> 16) & 0xFF);
				MMU_VRAMmapControl<3>((val >> 24) & 0xFF);
			break;

			case REG_VRAMCNTE:
				MMU_VRAMmapControl<4>(val & 0xFF);
				MMU_VRAMmapControl<5>((val >> 8) & 0xFF);
				MMU_VRAMmapControl<6>((val >> 16) & 0xFF);
				MMU_VRAMmapControl<7>((val >> 24) & 0xFF);
			break;
			case REG_VRAMCNTH:
				MMU_VRAMmapControl<8>(val & 0xFF);
				MMU_VRAMmapControl<9>((val >> 8) & 0xFF);
				break;
